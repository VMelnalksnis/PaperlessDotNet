	...
	private static KeyValuePair<string, string> ToKeyValuePair(this Expression expression)
	{
		...
		if (expression is MethodCallExpression methodCallExpression)
		{
			...
			if (methodCallExpression.Object is { } obj)
			{
				...
				var value = valueExpression.Evaluate() ?? throw new NullReferenceException();
			...
			{
				var value = methodCallExpression.Arguments[0].Evaluate() ?? throw new NullReferenceException();
				if (value is IEnumerable<int> values)
				{
					suffix = "in";
					value = string.Join(",", values);
